...
/*
//clickCart
let initialState0 = {ShowCart1:true}
function clickCartReducer(state=initialState0,action){
    switch(action.type){
        case 'CART':
            return{ShowCart1:!state.ShowCart1,dataChanged:false}
        default:
            return state;    
    }
}
...
function addOrRemoveItemReducer(state=initialState2,action){
    ...
    switch(action.type){
        ...
        case 'REMOVE_ITEMS':
            ...
            //const selProducts = JSON.parse(JSON.stringify(state.selectProducts));
            const pname=selProductsCopy[action.index].name;
            selProductsCopy[action.index].quantity=0;
            productsCopy[action.index].quantity = originalQty[action.index];
            if(state.searchMode){
                searchResultsCopy[0].quantity = originalQty[action.index];
            }   
...
export const rootReducer = combineReducers({addOrRemoveItemReducer,previewReducer})
